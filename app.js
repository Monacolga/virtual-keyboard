    ...
    row.forEach((key) => {
      ...
      keyElement.addEventListener('click', () => {
        if (keyElement.textContent === 'delete') {
          ...
          const { selectionStart } = output;
          const { selectionEnd } = output;
...
function createDescription() {
  ...
  descriptionElement.textContent = 'Клавиатура создана в операционной системе Mac IOS. Раскладка только английская, не все кнопки работают корректно (работа выполнена частично)';
...
document.addEventListener('keydown', (event) => {
  ...
  const matchingKey = Array.from(keyboardKeys).find(
    (key) => key.textContent.toLowerCase() === event.key.toLowerCase(),
  );
